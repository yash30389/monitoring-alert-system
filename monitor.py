...
import time
import json
from config import RDS_HOSTNAME, RDS_PORT, RDS_DATABASE, RDS_USERNAME, RDS_PASSWORD, RDS_DATABASE_ENGINE_TYPE, endpoints, WEBHOOK_URLS, SNS_TOPIC_ARN
from datetime import datetime

current_month = datetime.now().strftime('%B')
current_year = datetime.now().year

def publish_to_sns(message, subject="Alert Notification"):
    """
        ...
        if count >= 3:  # Trigger alert if the issue occurs 3 times in a short period
            if notification == "Unhealthy":
                ...
                alert_message = (
                    ...
                    f"Latest Response Time: {response_time}, Max Allowed: {max_response_time}"
                )
...
def main():
    ...
    try:
        ...
        print(f"Error in main operation: {e}")
